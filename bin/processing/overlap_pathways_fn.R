...
merge_gsea_results <- function(gsea_results_multi, ds, column_to_merge) {
  ...
  gsea_results_list <- map2(gsea_results_list, 1:length(gsea_results_multi), function(df, i) {
    df <- as.data.frame(df)
    df <- df[, c(1, 3, 5, 7)]
  ...
  if (length(gsea_results) < 2) {
    merged_gsea_results <- unlist(gsea_results_list)
  } else {
    merged_gsea_results <- reduce(gsea_results_list, function(df1, df2) {
...
# Function to merge pathway_results1 with the output of merge_gsea_results
#' Merge Pathway Results Across Datasets
#'
#' This function merges two data frames containing pathway results based on the column_to_merge column.
#'
#' @param pathway_results1 A data frame containing pathway results with a column_to_merge column.
#' @param pathway_results2 A data frame containing pathway results with a column_to_merge column.
#'
#' @return A data frame containing the merged pathway results from both input data frames.
#'
#' @examples
#' # Example usage:
#' # pathway_results1 <- data.frame(NAME = c("Pathway1", "Pathway2"), Value1 = c(1, 2))
#' # pathway_results2 <- data.frame(NAME = c("Pathway1", "Pathway3"), Value2 = c(3, 4))
#' # merged_results <- merge_pathway_results_across_datasets(pathway_results1, pathway_results2)
#' # print(merged_results)
#'
#' @export
merge_pathway_results_across_datasets <- function(pathway_results1, pathway_results2, column_to_merge) {
  all_pathways <- merge(pathway_results1, pathway_results2, by = column_to_merge)
